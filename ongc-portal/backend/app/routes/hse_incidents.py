...
from app.models.base import HSEIncident, User
...
async def _scope_query(db, user, section=None):
    ...
    if role_name == "ops_manager":
        ...
        q = q.where(HSEIncident.created_by.in_(managed_ids))
        return q
    q = q.where(HSEIncident.created_by == user.id)
    return q
...
async def list_items(
    ...
    return [{"id": x.id,             "date": str(x.date) if x.date else None,             "incident_type": x.incident_type,             "location": x.location,             "description": x.description,             "action_taken": x.action_taken} for x in items]
...
async def create_item(
    ...
    return {"id": obj.id, "msg": "hse incident created"}
...
async def update_item(
    ...
    if role_name not in ("admin", "ops_manager", "data_creator"):
        raise HTTPException(status_code=403, detail="Not enough permissions")
    result = await db.execute(select(HSEIncident).where(HSEIncident.id == item_id))
...
async def excel_preview(
    ...
    if title_header and title_header in headers:
        result = await db.execute(select(HSEIncident.description))
...
async def excel_import(
    ...
    valid_fields = {"action_taken","date","description","incident_type","location"}
