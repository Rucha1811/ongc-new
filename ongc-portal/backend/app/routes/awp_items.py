...
from app.models.base import AWPItem, User
...
COLUMN_SYNONYMS = {'activity': ['activity', 'task', 'work item', 'action', 'item'], 'target': ['target', 'goal', 'planned', 'objective'], 'achieved': ['achieved', 'accomplished', 'actual', 'done', 'completed'], 'progress': ['progress', '%', 'completion', 'percent'], 'deadline': ['deadline', 'due date', 'target date', 'end date', 'completion date'], 'status': ['status', 'state', 'health']}
...
async def _scope_query(db, user, section=None):
    ...
    q = select(AWPItem).order_by(AWPItem.created_at.desc())
    if role_name == "admin":
        return q
    if role_name == "ops_manager":
        ...
        q = q.where(AWPItem.created_by.in_(managed_ids))
        return q
    q = q.where(AWPItem.created_by == user.id)
    return q
...
async def create_item(
    ...
    return {"id": obj.id, "msg": "awp item created"}
...
async def update_item(
    ...
    if role_name not in ("admin", "ops_manager", "data_creator"):
        raise HTTPException(status_code=403, detail="Not enough permissions")
    result = await db.execute(select(AWPItem).where(AWPItem.id == item_id))
    ...
    if deadline is not None:
        obj.deadline = date_type.fromisoformat(deadline)
...
async def excel_preview(
    ...
    title_header = next((k for k,v in auto_mapping.items() if v=="activity"), None)
    existing = set()
    dup_rows = 0
    if title_header and title_header in headers:
        result = await db.execute(select(AWPItem.activity))
