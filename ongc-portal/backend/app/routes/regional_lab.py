...
from app.models.base import RegionalLabEquipment, User
...
COLUMN_SYNONYMS = {'section': ['section', 'tab', 'lab group', 'group', 'lab'], 'equipment': ['equipment', 'instrument', 'device', 'asset', 'item'], 'status': ['status', 'condition', 'operational status'], 'last_calibration': ['last calibration', 'last_calibration', 'calibration date', 'last serviced'], 'next_due': ['next due', 'next_due', 'next calibration', 'due date']}
...
async def _scope_query(db, user, section=None):
    ...
    q = select(RegionalLabEquipment).order_by(RegionalLabEquipment.created_at.desc())
    if role_name == "admin":
        if section:
            q = q.where(RegionalLabEquipment.section == section)
        return q
    if role_name == "ops_manager":
        ...
        if ms:
            q = q.where(or_(RegionalLabEquipment.section.in_(ms), RegionalLabEquipment.created_by.in_(managed_ids)))
        else:
            q = q.where(RegionalLabEquipment.created_by.in_(managed_ids))
        return q
    sec = section or user.section or user.area
    if sec:
        q = q.where(RegionalLabEquipment.section == sec)
    else:
        q = q.where(RegionalLabEquipment.created_by == user.id)
    return q
...
async def create_item(
    ...
    last_calibration: str = Form(None),
    ...
    return {"id": obj.id, "msg": "regional lab created"}
...
async def update_item(
    ...
    if role_name not in ("admin", "ops_manager", "data_creator"):
        raise HTTPException(status_code=403, detail="Not enough permissions")
    result = await db.execute(select(RegionalLabEquipment).where(RegionalLabEquipment.id == item_id))
    obj = result.scalar_one_or_none()
    if not obj:
        raise HTTPException(404, "regional lab not found")
...
async def excel_preview(
    ...
    if title_header and title_header in headers:
        result = await db.execute(select(RegionalLabEquipment.section))
